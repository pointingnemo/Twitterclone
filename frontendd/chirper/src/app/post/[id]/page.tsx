...
interface combinedProps extends PostProps ,WriteCommentProps {}
const PostPage: React.FC<combinedProps> = ({ params }) => {
  const [post, setPost] = useState<postProps | null>(null);
  const [newComment, setNewComment] = useState<string>('');
  const [comments, setComments] = useState(post?.comments || []); 
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (newComment.trim() === '' || postId === null) return;

    try {
      const response = await fetch(`http://localhost:5000/api/post/comment/${postId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <>
            ...
            <div className="mt-4 ml-5">
              <form onSubmit={handleCommentSubmit} className="flex items-center">
                ...
                <button
                  type="submit"
                  className="ml-2 h-8 px-3 bg-indigo-500 text-white rounded hover:bg-indigo-800 text-sm"
                >
                  Post
                </button>
              </form>
